...
function getSystemStatus() {
    ...
    #Rongzheng.tanghuang@CONNECTIVITY.WIFI, 2021/10/22, Add for dumpsys wifi & network info
    dumpWifi
...
#Rongzheng.tanghuang@CONNECTIVITY.WIFI, 2021/10/22, Add for dumpsys wifi & network info
function dumpWifi() {
    wifiPath="${DATA_DEBUGGING_PATH}/SI_stop/wifi"
    mkdir -p ${wifiPath}
    traceTransferState "dumpWifi:start...."
    dumpsys wifi >  ${wifiPath}/wifi_dump.txt
    dumpsys connectivity > ${wifiPath}/connectivity.txt
    dumpsys network_stack> ${wifiPath}/network_stack.txt
    dumpsys netpolicy> ${wifiPath}/netpolicy.txt
    dumpsys oplusnetd firewall > ${wifiPath}/oplusnetd_firewall.txt
    dumpsys network_management > ${wifiPath}/network_management.txt
    netstat > ${wifiPath}/netstat.txt
    iptables -L -n -v > ${wifiPath}/iptable_Lnv.txt
    iptables -t nat -nvL > ${wifiPath}/iptables_nat.txt

    ip rule show > ${wifiPath}/v4_ip_rule_show.txt
    ip route show table all >  ${wifiPath}/v4_ip_route_show_table_all.txt
    ip addr > ${wifiPath}/ip_addr.txt
    ifconfig > ${wifiPath}/ifconfig.txt
    traceTransferState "dumpWifi:done...."
}
